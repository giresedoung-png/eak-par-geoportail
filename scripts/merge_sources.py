...
def dedupe_kobo_vs_legacy(kobo_rows, legacy_rows):
    """Recherche les PAP potentiellement présentes à la fois dans les données
    Kobo (temps réel) et dans les données papier historiques (même personne
    saisie deux fois : une fois sur papier, une fois re-vérifiée via Kobo).

    ⚠️ IMPORTANT : la fusion automatique a été désactivée. Même un nom
    strictement identique dans le même village ne garantit pas qu'il s'agisse
    de la même personne (homonymie possible) — seule une vérification terrain
    peut le confirmer avec certitude. Le script se contente donc de
    SIGNALER les correspondances possibles dans le journal d'exécution, sans
    jamais retirer une PAP du décompte total. Un doublon non détecté (compté
    deux fois) est un moindre mal largement préférable à une vraie PAP
    distincte supprimée à tort.
    """
    ...
    for lrow in legacy_rows:
        ...
        if meilleur_match and meilleur_score >= SEUIL_SIGNALEMENT:
            ...
            a_verifier_manuellement.append(
                ...
                f"score {meilleur_score:.2f} — conservées séparées, à vérifier manuellement si besoin"
            )

    if a_verifier_manuellement:
        print(f"\n⚠ {len(a_verifier_manuellement)} correspondance(s) possible(s) entre papier et Kobo "
              f"détectée(s) — AUCUNE fusion automatique effectuée (toutes les PAP restent comptées "
              f"séparément) ; à vérifier manuellement si un doublon terrain avéré existe :")
        for ligne in a_verifier_manuellement:
            print(f"    - {ligne}")

    return legacy_rows  # aucune ligne retirée : le décompte total reste exact
